  ...
  export default function CargoForm(props){
    ...
    useEffect( () => {
        const fetchRegistro = async (id) => {
            const result = await api.get(`/cargo-by-id/${id}`)  
                    setData(result.data);                       
    ...
    const onSubmit = async data => {
        ...
        const { dsc_cargo } = data;   

        if(props.id){             
            try {
                await api.patch(`/cargos/${props.id}`, { dsc_cargo });
                history.push("/cargos"); 
                Toast.fire({
                    icon: 'success',
                ...
                await api.post("/cargos", { dsc_cargo });            
                history.push("/cargos"); 
                Toast.fire({
                    icon: 'success',
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
            ...
            <div>
                ...
                <TextField
                    ...
                    inputRef={register}
                    name="dsc_cargo"
                    InputLabelProps={{shrink: Boolean(campos.dsc_cargo)}}                   
                />
